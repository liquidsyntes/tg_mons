...
- Первичный бэкфельд истории на 30 дней при добавлении канала, далее полный рефреш последних 200 постов каждый цикл с дедупликацией (позволяет обновлять просмотры и реакции существующих постов).
...
Route handler'ы API (мутирующие и внутренние маршруты используют Bearer-токен `COLLECT_API_TOKEN` там, где это требуется):
...
| `WEB_INTERNAL_URL` | для сброса кэша worker'ом | URL web-процесса для `POST /api/internal/invalidate-cache` |
...
2. **Дельты при неполной истории:** вместе со значением возвращается фактическое покрытие в днях; при заметном отличии от номинального периода UI показывает его рядом с дельтой.
