...
const replacement = `} else if (report.type === 'persona') {
      contentHtml = \`
...
const realReplacement = replacement.replace(/\\`/g, '`');

content = content.replace(/\} else if \(report\.type === 'persona'\) \{[\s\S]*?\} else \{/, realReplacement);
