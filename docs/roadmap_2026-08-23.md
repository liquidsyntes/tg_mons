...
## 11. Дашборд с агрегированными трендами по нише

**Проблема:** На главной — список каналов. Нет агрегированных метрик: средний рост по нише, общий охват, тренд постинга.

```
Доработай главную страницу TgMon. Сейчас это таблица каналов. Добавь дашборд с агрегированными метриками.

1. Создай src/lib/dashboard.ts:
   - getDashboardStats():
     - totalChannels, totalSubscribers, totalPosts30d
     - avgGrowthRate (средний % роста по всем каналам за 7d)
     - avgErr (средний ERR по всем каналам)
     - topGainers: топ-5 каналов по Δ7d (abs)
     - topLosers: топ-5 каналов по Δ7d (отрицательный)
     - postsTimeline: количество постов по дням за 30д (агрегат по всем каналам)
     - subscribersTimeline: сумма подписчиков по дням (из снапшотов)

2. API endpoint GET /api/stats/dashboard — возвращает getDashboardStats().

3. UI: src/components/Dashboard.tsx (новая главная страница):
   - Верхняя строка: 4 KPI-карточки (всего каналов, всего подписчиков, ср. рост 7d, ср. ERR)
   - LineChart: сумма подписчиков всех каналов по дням
   - BarChart: публикационная активность по дням (все каналы)
   - TopGainers / TopLosers: две колонки с мини-карточками
   - Существующая ChannelsTable — ниже, под дашбордом
   - Используй кэширование (metricsCache, ключ 'dashboard', TTL 5 мин)

4. Размести дашборд над ChannelsTable на главной странице (/).
```
